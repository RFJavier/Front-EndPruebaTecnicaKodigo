...
import { ref } from "vue";
import { user_data } from '../types/auth.types';
import { Field, Form, useForm} from "vee-validate";
const showModal = ref(false);
const { CreateUser } = UseAuthStore();
const rol = 2;
const active = 1;
const { handleSubmit } = useForm<user_data>({
  validationSchema: yup.object().shape({
    idrol: yup.number(),
    username: yup.string().required("Debe escribir su nombre"),
    nickname: yup.string().required("Debes escribir el correo"),
    userpassword: yup.string().required("Debes escribir tu contraseña"),
    email: yup.string().required("Debe escribir un correo"),
    isactive: yup.number(),
    confirmPassword_aux: yup.string(),
  }),
});
